...
from beat_this.custom_vqt.custom_preprocessing import load_audio, LogMelSpectTorch, CQTNotesSpectLibrosa, CQTNotesSpectNN, VQTNotesSpectLibrosa, VQTNotesSpectCustom, VQTNotesSpectNN
...
def load_model(
    ...
    # model state dict
    pretained_state = model.state_dict()

    # custom model
    custom_model = BeatThis(spect_dim=96)
    custom_state = custom_model.state_dict()

    # extract only unmodified structures
    for k in custom_state:
        if k in pretained_state and custom_state[k].shape == pretained_state[k].shape:
            custom_state[k] = pretained_state[k]
